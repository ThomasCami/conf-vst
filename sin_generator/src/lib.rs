...
impl Plugin for DevFestSynth {
    const NAME: &'static str = "DevFest Sin Generator";
    ...
    fn process(
        ...
        for (_sample_id, channel_samples) in buffer.iter_samples().enumerate() {
            ...
            let sin = self.calculate_sin(frequency);
...
impl ClapPlugin for DevFestSynth {
    const CLAP_ID: &'static str = "DevFest Sin Generator";
    const CLAP_DESCRIPTION: Option<&'static str> =
        Some("A sine test tone");
...
impl Vst3Plugin for DevFestSynth {
    const VST3_CLASS_ID: [u8; 16] = *b"DevFestSinGenera";
